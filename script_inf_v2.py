...
from excel_export.excel_export_inf_v2 import to_excel, openpyxl_formatting, to_excel_v2, openpyxl_formatting_v2  # noqa
from parameters.parametres_inf_v2 import I, J, K, nb_semaines, part_time_I, full_time_I  # noqa
import os
...
def populate_by_row(prob):
    ...
    for j in J:
        ...
        if j % 6 != 0:
            ...
            prob += lpSum(x[i, j, 2] for i in I) >= 2 #+ lpSum(x[i, j, 4] for i in part_time_I)
            # C3 (day shift constraint)
            prob += lpSum(x[i, j, 3] for i in I) >= 1 #+ lpSum(x[i, j, 4] for i in part_time_I)
        # Weekends
        else:
            # C1 (morning shift constraint)
        ...
        for i in I:
            ...
            prob += lpSum(x[i, j, k] for k in K[:-1]) <= 1
    ...
    for i in I:
        ...
        for j in range(1, nb_semaines):
            prob += lpSum(x[i, j * 6, k] + x[i, (j + 1) * 6, k] for k in K[:-1]) == 1
        # C8 (no evening-rest-morning sequence)
        for j in range(2, len(J)):
            prob += lpSum(x[i, j, k] - x[i, j-1, k] - x[i, j + 1, k] for k in K[:-1]) <= 0
        # C9 (at least a day off per week)
        for j in range(1, len(J) - 5 + 1):
            prob += lpSum(x[i, j + index, k] for k in K[:-1] for index in range(5)) <= 4
        ...
        prob += (lpSum(x[i, j, k] * (1 + int(j % 6 == 0)) for j in J for k in K[:-1])
    ...
    for i in I:
        for j in J[:-2]:
            prob += lpSum(x[i, j + 1, k] for k in K[:-1]) - x[i, j, 2] - x[i, j + 2, 1] >= -1
    ...
    for idx, i in enumerate(I[:-1]):
        ...
        for j_idx, j in enumerate(J):
            ...
            #     for k in K[:-1]:
            #         prob += (x[i, j, k] == x[next_i, next_j, k])
            # elif i in part_time_I:
            #     for k in K:
            #         prob += (x[i, j, k] == x[next_i, next_j, k])
    # C14: no more than 3 consecutive days off
    for i in I:
        for j in J[:-2]:
            prob += lpSum(x[i, j + index, k] for k in K[:-1] for index in range(3)) <= 3

    # Part-time agents
    # for i in I:
    for i in part_time_I:
        # C15 (One day pinned per week except for weekends)
        prob += lpSum(x[i, j, 4] for j in J[:6] if j % 6 != 0) == 1
        prob += lpSum(x[i, j, 4] for j in J[:6] if j % 6 == 0) == 0

        # C16 (Day pinned must not be a day off)
        for j in J:
            prob += x[i, j, 4] - lpSum(x[i, j, k] for k in K[:-1]) <= 0
    for i in full_time_I:
        prob += lpSum(x[i, j, 4] for j in J[:6]) == 0
    return x
