...
import pandas as pd
from parameters.parametres_inf_v2 import I, J, K  # , part_time_I, full_time_I, nb_part_time_agents
...
def to_excel_v2(values, variable_names, dest_path="export/nurse_schedule_v2.xlsx"):
    ...
    for i in I:  # For each nurse
        ...
        for j in J:  # For each day
            ...
            for k in K[:-1]:  # For each shift
                index = variable_names.index(f"x{i},{j},{k}")
                if values[index] == 1:
                    shift_value = shifts[k]
            if values[variable_names.index(f"x{i},{j},4")] == 1:
                shift_value += " (RA)"
    ...
    shift_names = {"M": "M", "S": "S", "T": "T", "M (RA)": "M", "S (RA)": "S", "T (RA)": "T", "R": "R"}
    df['Total'] = df.apply(lambda row: sum([1 for shift in row if shift in shift_names and shift != "R"]), axis=1)
    total_nurses = df.apply(lambda col: sum([1 for shift in col if shift in shift_names and shift != "R"]), axis=0)
    total_nurses.name = 'Total'
    df = pd.concat([df, total_nurses.to_frame().T])
    for shift in ["M", "S", "T"]:
        df[f'Total {shift}'] = df.apply(lambda row: sum([1 for s in row
                                                         if (s in shift_names.keys() and shift_names[s] == shift)]),
                                        axis=1)
        total_nurses = df.apply(lambda col: sum([1 for s in col
                                                 if (s in shift_names.keys() and shift_names[s] == shift)]),
                                axis=0)
